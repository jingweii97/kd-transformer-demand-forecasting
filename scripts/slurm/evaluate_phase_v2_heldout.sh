#!/bin/bash -l
# Held-out protocol is unchanged; this only evaluates selected v2 artifacts.
#SBATCH --partition=gpu-a100
#SBATCH --job-name=eval_phase_v2_heldout
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --mem=32G
#SBATCH --qos=long
#SBATCH --output=logs/slurm/eval_phase_v2_heldout_%j.out
#SBATCH --error=logs/slurm/eval_phase_v2_heldout_%j.err
set -euo pipefail
cd "${SLURM_SUBMIT_DIR:-$PWD}"
OUT="outputs/evaluation/phase_v2_validation_selected_heldout"
[[ ! -e "$OUT" ]] || { echo "Refusing output overwrite: $OUT"; exit 1; }
teacher_run="outputs/teacher/phase_teacher_v2"
wis_run="outputs/student/no_kd/phase_wis_v2"
wikd_run="outputs/student/kd/phase_wikd_v2"
...
python scripts/evaluate_models.py --env dicc --experiment phase_wikd_v2 --exp-name phase_v2_validation_selected_heldout --teacher-checkpoint "${teacher_selected[0]}" --student-nokd-checkpoint "${wis_selected[0]}" --student-kd-checkpoint "${wikd_selected[0]}" --selected-student-label "Student-WIKD v2"
